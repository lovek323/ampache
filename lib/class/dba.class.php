...
class Dba
{
     ...
     * @param PDOStatement|bool $resource
     * @param bool $finish
     * @return array
     */
    public static function fetchAssoc($resource, $finish = true)
    {
        if (!$resource) {
            return [];
        }
     ...
     * @return array|mixed
     */
    public static function fetch_object($resource, $class = stdClass::class, $finish = true)
    {
        if (!$resource) {
            return [];
        }
     ...
     * @param bool|PDOStatement $resource
     * @return int
     */
     ...
     * @return bool|null
     */
    private static function setupDbh($dbh, $database)
    {
        if (!$dbh) {
            return false;
        }
    ...
    public static function reset_db_charset()
    {
        ...
        $targetCharset = $translated_charset['charset'];
        $target_collation = $translated_charset['collation'];

        // Alter the charset for the entire database
        $sql = "ALTER DATABASE `" . AmpConfig::get('database_name') . "` DEFAULT CHARACTER SET $targetCharset COLLATE $target_collation";
        Dba::write($sql);
        ...
        while ($row = Dba::fetch_row($db_results)) {
            ...
            $sql = "ALTER TABLE `" . $row['0'] . "`  DEFAULT CHARACTER SET $targetCharset COLLATE $target_collation";
            Dba::write($sql);

            // Iterate through the columns of the table
            while ($table = Dba::fetchAssoc($describe_results)) {
                if (
...
EOSQL;
                    ...
                    if (!$charset_results) {
                        debug_event(
                            'CHARSET',
                            "Unable to update the charset of {$table['Field']}.{$table['Type']} to {$targetCharset}",
                            '3'
                        );
                    } // if it fails
                }
